function Yn_u = Observed_data_processing_u_all (time, Num_of_rp5,N)
...
Yn_u = cell(Num_of_rp5,5);

% start and end time in original WRF file
time_start = time(1)  *60 + 6*60*60; 
time_end   = time(end)*60 + 6*60*60; 
Subset1 = datetime(time_start,'ConvertFrom','epochtime','Epoch','2012-01-01');   % Start of Month {e.g.'01-Aug-2014 00:00:00'}
Subset2 = datetime(time_end,  'ConvertFrom','epochtime','Epoch','2012-01-01');   % End of Month {e.g.  '31-Aug-2014 23:00:00'}
...
try
    ...
    if isempty(Index1) == 1  % We failed to find an Index1
        i = 1;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Yn_u{g,1} = rot90(U_i); % Time-serie itself - raw data
    Yn_u{g,2} = rot90(T_i); % Time instances for this time-serie  - raw data
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
        if isempty(Yn_u{g,1}) == 1 % we have not found any
            Yn_u{g,1} = zeros(1,length(time));
            Yn_u{g,2} = rot90(time_conv);
        end
    
    t1 = datevec(Yn_u{g,2}); % Time atributes of the observed array in timevec format (отделяем часы и дни (1,2,3 ...) )
    for i = 1:length(t1)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if length(Yn_u{g,2}) ~= N % Если есть пропуски - делаем resampling
    fs = 1;
